...
(deftest examples-1
  (is (password-valid? "aa bb cc dd ee"))
  (is (not (password-valid? "aa bb cc dd aa")))
  (is (password-valid? "aa bb cc aaa")))

;;
(deftest result-1
  (is (= 383 (count-valid-passwords input))))
...
(defn password-extra-valid?
  [pass]
  (let [spl (map sort-word (str/split pass #" "))
        oc (count spl)
        sc (count (set spl))]
    (= oc sc)))

(defn count-extra-valid-passwords
  [input]
  (->> input
       (map password-extra-valid?)
       (frequencies)
       (#(get % true))))

(deftest examples-2
  (is (password-extra-valid? "abcde fghij"))
  (is (not (password-extra-valid? "abcde xyz ecdab")))
  (is (password-extra-valid? "a ab abc abd abf abj"))
  (is (password-extra-valid? "iiii oiii ooii oooi oooo"))
  (is (not (password-extra-valid? "oiii ioii iioi iiio"))))

(deftest result-2
  (is (= 265 (count-extra-valid-passwords input))))
